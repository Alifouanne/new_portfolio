...
interface AuroraBackgroundProps extends React.HTMLProps<HTMLDivElement> {
  children?: ReactNode; // Children are optional as it's primarily a background
...
export const AuroraBackground = ({
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 overflow-hidden transition-colors duration-500", // Fixed, covers viewport, and transitions its own background
        "bg-zinc-100 dark:bg-zinc-900", // Adjusted to bg-zinc-100 for light mode for more contrast
        className,
      )}
      {...props}
    >
      ...
      {children} {/* Render children on top of the background */}
    </div>
  );
};
